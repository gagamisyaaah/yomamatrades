...
def confirmed(d: pd.Series, high: pd.Series, low: pd.Series, close: pd.Series) -> pd.Series:
    """±1 once the diamond bar's high (blue) / low (pink) is closed through within 3 bars; 0 if invalidated / none."""
...
def states(df: pd.DataFrame) -> pd.DataFrame:
    ...
    echo = diamond_engine(hype, hype_sig)
    tango = diamond_engine(flow, flow_sig)
    ...
    c_e, c_t, c_b = confirmed(echo, h, l, c), confirmed(tango, h, l, c), confirmed(bravo, h, l, c)
    ...
    out["mountain"] = (hype > 0).astype(int) + (flow > 0).astype(int) + (c > gold_top).astype(int) + ((e1 > e3) & (e1 > e1.shift(1))).astype(int)
