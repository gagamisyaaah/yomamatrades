"""Jev scores the longlist (bend/evo/models_longlist.txt): usefulness, portability, family, documented. → bend/data/models_jev2.md/.csv"""
...
import pandas as pd
rows = []
for ln in open("bend/evo/models_longlist.txt"):
    if ln.startswith("#") or "|" not in ln: continue
    dom, name, desc = [x.strip() for x in ln.split("|", 2)]
    try:
...
D = D.sort_values("rank", ascending=False); D.to_csv("bend/data/models_jev2.csv", index=False)
md = lambda d: "| " + " | ".join(map(str, d.columns)) + " |\n|" + "---|" * len(d.columns) + "\n" + "\n".join("| " + " | ".join(str(v) for v in r) + " |" for r in d.itertuples(index=False))  # noqa: E731
open("bend/data/models_jev2.md", "w").write(f"# Jev over {len(D)} cross-field models: useful × portable × (½ + ½ documented)\n\n" + md(D[["domain", "model", "family", "useful", "portable", "documented", "rank", "measurement"]]) + "\n")
